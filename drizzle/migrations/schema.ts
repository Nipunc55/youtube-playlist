import { mysqlTable, mysqlSchema, AnyMySqlColumn, primaryKey, unique, int, varchar, timestamp, index } from "drizzle-orm/mysql-core"
...
(table) => {
	return {
		likesLikeIdPk: primaryKey({ columns: [table.likeId], name: "likes_like_id_pk"}),
	}
});
...
(table) => {
	return {
		usersUserIdPk: primaryKey({ columns: [table.userId], name: "users_user_id_pk"}),
	}
});
